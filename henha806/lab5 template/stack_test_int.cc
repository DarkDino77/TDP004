...
SCENARIO( "Lists can be heavily used" )
{
    ...
    GIVEN( "A list with 1000 random items in it" )
    {
        ...
        WHEN( "the list have 1000 random numbers pushed" )
        {
            for (int i{0}; i < 1000; ++i)
            {
                int random { uniform(rd) }; // generate a random number
            ...
            THEN("the list has 2000 items in it")
            {
                REQUIRE(l.size() == 2000);
            }
        }

        WHEN( "the list have 2000 random numbers added then removed" )
        {
            for (int i{0}; i < 1000; ++i)
            {
                int random { uniform(rd) }; // generate a random number
...
Stack<int> trigger_move(Stack<int> l)
{
	l.push(5);
	l.push(9);
	l.push(3);
...
void use_const_list(Stack<int> const& l)
{
    l.is_empty();
    l.size();
