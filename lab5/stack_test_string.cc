...
SCENARIO( "Multi-item lists" )
{
    ...
    GIVEN( "A list with five items in it" )
    {
		...
		{
			l.pop();
			THEN( "The remaining items are still in the list and the corresponding postd::stringers are corrected" )
			{
...
Stack<std::string> trigger_move(Stack<std::string> l)
{
	l.push(5);
	l.push(9);
	l.push(3);
...
SCENARIO( "Lists can be passed to functions" )
{
    ...
    GIVEN( "A list with 5 items in it" )
    {
		...
		{
			Stack<std::string> l{ trigger_move(given) };
...
void use_const_list(Stack<std::string> const& l)
{
    l.is_empty();
    l.size();
...
SCENARIO( "All operations that do not modify the list can be performed on a list" )
{
    GIVEN( "A list with five items in it" )
    {
        ...
        WHEN( "The list is sent std::stringo the modify function" )
        {
